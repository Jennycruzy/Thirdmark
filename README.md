...
to deploy Thirdmark. Lace owns wallet synchronization, balancing, signing, and
submission; the laptop does not need to run the headless reference-counter wallet
replay. The deployment action remains disabled without the public issuer key and it
does not bypass wallet or network safety checks.

The browser runtime requires the four full proving artifacts generated by the managed
Compact compile (`file.prover`, `file.verifier`, `file.bzkir`, and `file.zkir`). They
are staged by [`scripts/prepare-browser-artifacts.sh`](scripts/prepare-browser-artifacts.sh)
in CircleCI and are intentionally not committed. The public build still has no
contract address, issuer endpoint, issuer public key, or registry-adapter fallback.
