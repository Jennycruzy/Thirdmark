...
- A later source inspection of the official iCRP frontend bundle on 2026-09-14 found the public-search request used by CAC itself: `POST https://authapp.cac.gov.ng/name_similarity_app/api/public_search/search` with `SearchType` and `searchTerm`, plus the iCRP `Origin` and `Referer` headers. A request without those browser-origin headers returned HTTP 403; the source-matched request returned HTTP 200 and real CAC records. The official response includes `approvedName`, `rcNumber`, `classificationName`, and `status`.
- Source correction: the earlier statement that no public-search request path could be used was too strong. The official frontend does expose a source-backed request path, but it is not a versioned third-party API and is rate-limited. Wave 1 now uses the narrow server-side adapter in [`registry/server.ts`](../registry/server.ts), which filters to `COMPANY` records and sends only `{ name, rcNumber, status }` to the browser. It stores no registry records and holds no CAC credential.
- Decision: Wave 1 uses CAC company RC numbers only. The adapter is suitable for the buildathon integration; production use should migrate to an authorized CAC VAS integration when available.
...
## Superseded product private-history bound

The Compact sources inspected for the selected toolchain use `List` as a ledger ADT;
no reference contract supplies an unbounded private witness list. The first
Thirdmark circuit therefore represented the private filing history as a bounded
witness vector with a private length and per-entry commitment salts. That design is
retained here as historical evidence only. It was superseded after 1AM rejected the
delegated proof payload even after the vector was reduced from 32 to 4 entries.

The current Wave 1 circuit removes that history vector and its public commitment
map. It keeps the scoped filer nullifier, ciphertext replay guard, threshold
transition, and private OPRF/DLEQ witnesses. The replacement passed CircleCI
pipeline `#30` with a 21,579,079-byte `file.prover` artifact, compared with
42,686,064 bytes for the four-entry circuit. This is the deliberate Wave 1
tradeoff until a smaller private-history representation is verified against the
wallet connector limit.
...
`entries`, `filed`, and `unlocked`; the generated implementation at
...
The filing client stages fresh OPRF material before proof generation. If finalization
fails, the prior state is restored; after finalization, the staged OPRF material is
